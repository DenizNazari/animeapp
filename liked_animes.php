...
// Fetch anime IDs liked by the user
$select_like_animes = mysqli_query($conn, "SELECT `animes_id` FROM `likes` WHERE `user_id` = {$_SESSION['id']} AND `rate` = 1");
$anime_ids = [];
while ($row = mysqli_fetch_assoc($select_like_animes)) {
    $anime_ids[] = $row['animes_id'];
}
$anime_ids_string = implode(",", $anime_ids);
$category = "WHERE `animes`.`id` IN ($anime_ids_string)";

// Count total records
$result_count = mysqli_query($conn, "SELECT COUNT(DISTINCT `animes`.`animecode`) AS total_records FROM `animes` LEFT JOIN `genres` ON `animes`.`animecode` = `genres`.`animeid` LEFT JOIN `themes` ON `animes`.`animecode` = `themes`.`animeid` " . $category);

if ($result_count) {
    $total_records_array = mysqli_fetch_array($result_count);
    $total_records = $total_records_array['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
    $second_last = $total_no_of_pages - 1; // total page minus 1

    // Fetch data for the current page
    $result = mysqli_query($conn, "SELECT DISTINCT `animes`.* FROM `animes`  " . $category );
}
